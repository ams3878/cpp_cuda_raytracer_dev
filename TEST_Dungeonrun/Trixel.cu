...
//#define inplace_device_cross(ax, ay, az, bx, by, bz) device_cross(&ax,&ay, &az,  ax,  ay,  az,  bx,  by,  bz)
__device__ void device_cross(double* cx, double* cy, double* cz, double ax, double ay, double az, double bx, double by, double bz) {
	*cx = ay * bz - az * by;
	*cy = az * bx - ax * bz;
	*cz = ax * by - ay * bx;
}
__device__ double device_inverse_sqrt(double x, double y, double z) {
	union { double x; s64 i; } u;
	u.x = (x * x) + (y * y) + (z * z); //(float)vector_normsq(v); //this cant be negative
	if (u.x < EPSILON) { return 0.0; } //not sure if i need to do anything, if errors change this
...
__device__ void device_normalize_vector(double* x, double* y, double* z) {
	double i_sqrt = device_inverse_sqrt(*x, *y, *z);
	*x *= i_sqrt; *y *= i_sqrt; *z *= i_sqrt;
}

__device__ double device_dot(double ax, double ay, double az, double bx, double by, double bz) {
...
__global__ void init_tri_mem_cuda(Trixel::trixel_memory* t, double* point_data) {
	...
	device_normalize_vector(&t->d_edges.e1.x[i], &t->d_edges.e1.y[i], &t->d_edges.e1.z[i]);
	...
	device_normalize_vector(&t->d_edges.e2.x[i], &t->d_edges.e2.y[i], &t->d_edges.e2.z[i]);
...
__global__ void init_cam_tri_mem_cuda(Trixel::trixel_memory* tm, Camera::trixel_memory* cm, Camera::orientation_properties::position c_pos) {
	...
	device_cross(&cm->d_q.x[i], &cm->d_q.x[i],&cm->d_q.x[i], cm->d_t.x[i], cm->d_t.y[i], cm->d_t.z[i], tm->d_edges.e2.x[i], tm->d_edges.e2.y[i], tm->d_edges.e2.z[i]);
	cm->d_w[i] = device_dot(cm->d_q.x[i], cm->d_q.x[i], cm->d_q.x[i], tm->d_edges.e2.x[i], tm->d_edges.e2.y[i], tm->d_edges.e2.z[i]);
...
__global__ void intersect_tri_cuda(Trixel::trixel_memory* tm, Camera::pixel_memory* cm, Camera::trixel_memory* ctm, u64 tcount) {
	u64 i = (u64)threadIdx.x + ((u64)blockIdx.x * blockDim.x);
	u64 tri_index = -1;
	double d = 400;//**TODO** get rid of hardcode
	double pe1, p_x, p_y, p_z, u, v, w;
	...
	for (u64 tri_i = 0; tri_i < tcount; tri_i++) {
		device_cross(&p_x, &p_y, &p_z,
			...
			tm->d_edges.e2.x[i], tm->d_edges.e2.y[i], tm->d_edges.e2.z[i]);
		pe1 = device_dot(p_x, p_y, p_z,
			tm->d_edges.e1.x[i], tm->d_edges.e1.y[i], tm->d_edges.e1.z[i]);
		if (!(pe1 < DEVICE_EPSILON_SINGLE && pe1 > -DEVICE_EPSILON_SINGLE)) {
			pe1 = 1.0 / pe1;
			u = pe1 * device_dot(p_x, p_y, p_z,
				ctm->d_t.x[i], ctm->d_t.y[i], ctm->d_t.z[i]);
			v = pe1 * device_dot(cm->rmd.d_x[i], cm->rmd.d_y[i], cm->rmd.d_z[i],
				ctm->d_q.x[i], ctm->d_q.y[i], ctm->d_q.z[i]);
			w = pe1 * ctm->d_w[i];
	...
	if (tri_index != (s32)-1) {
		...
		cm->d_color.c[i] = tm->d_color.c[i];
		cm->norm.d_x[i] = tm->d_n.x[i];
		cm->norm.d_y[i] = tm->d_n.y[i];
		cm->norm.d_z[i] = tm->d_n.z[i];
	}
}
...
cudaError_t init_trixels_device_memory(Trixel* t) {
	...
	init_tri_mem_cuda << < t->num_trixels / bl_size, bl_size >> > ((Trixel::trixel_memory *) t->d_mem, t->d_points_init_data);
